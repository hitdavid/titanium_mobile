	...
	return declare("Ti._.Layouts.ConstrainingVertical", Base, {
		...
		_doLayout: function(element, width, height, isWidthSize, isHeightSize) {
			...
			for(i = 0; i < len; i++) {
				...
				child = element._children[i];
				//Desc :  recalculate the height , using node flex prop.
				//Modified by : yangxk@chanjet.com
				if (flexCount) {
					runningHeight = Math.floor(remainingSpace / flexCount) * child._flex;
				}
				...
				if (child._markedForLayout) {
					...
					if (heightLayoutCoefficients.x2 !== 0 && !isNaN(heightLayoutCoefficients.x2)) {
						...
						measuredWidth = widthLayoutCoefficients.x1 * width + widthLayoutCoefficients.x2;
						measuredHeight = heightLayoutCoefficients.x1 * height + heightLayoutCoefficients.x2 * ((i < len - 1 || child._flex)? runningHeight : remainingSpace - runningHeight * (fillCount - 1)) + heightLayoutCoefficients.x3;
